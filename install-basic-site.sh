...
BASIC_SITE_CONF_FILE="basic-site.conf"
...
BASIC_SITE_HOST_IP=$(grep $BASIC_SITE_HOST_IP_PATTERN $CONF_FILES_DIR/$BASIC_SITE_CONF_FILE | awk -F "=" '{print $2}')
echo "Basic site host ip: $BASIC_SITE_HOST_IP"

BASIC_SITE_PRIVATE_KEY_FILE_PATH_PATTERN="basic_site_host_ssh_private_key_file"
BASIC_SITE_PRIVATE_KEY_FILE_PATH=$(grep $BASIC_SITE_PRIVATE_KEY_FILE_PATH_PATTERN $CONF_FILES_DIR/$BASIC_SITE_CONF_FILE | awk -F "=" '{print $2}')
...
REMOTE_USER=$(grep $REMOTE_USER_PATTERN $CONF_FILES_DIR/$BASIC_SITE_CONF_FILE | awk -F "=" '{print $2}')
