...
function install_system_dependencies {
        sudo apt-get update
        sudo apt-get install apt-transport-https ca-certificates curl gnupg-agent software-properties-common python -y
}
...
function run_atomix_container {
  ...
  containerID="$(sudo docker create --name atomix_node -p 5679:5679 atomix/atomix:$atomix_version -c $path/$config_fn)"
