...
sudo docker run -idt \
	--name $CONTAINER_NAME \
	-p $RAS_PORT:8080 \
	-v ./$CONF_FILES_DIR:$CONTAINER_BASE_PATH/$CONTAINER_CONF_FILES_DIR \
	-v ./$APPLICATION_CONF_FILE_NAME:$CONTAINER_BASE_PATH/$CONTAINER_RESOURCES_PATH/$APPLICATION_CONF_FILE_NAME \
	-v ../$TIMESTAMP_DB_DIRECTORY_NAME/$RAS_TIMESTAMP_DB_FILE_NAME:$CONTAINER_DB_FILE_PATH\
	$IMAGE_NAME:$TAG
