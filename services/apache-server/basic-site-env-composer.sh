...
BASIC_SITE_HOST_IP=$(grep $BASIC_SITE_HOST_IP_PATTERN $CONF_FILES_DIR/$BASIC_SITE_CONF_FILE_NAME | awk -F "=" '{print $2}')
...
sed -i "s|$PROVIDER_ID_PATTERN|$PROVIDER_ID|g" $BASE_DIR/$VIRTUAL_HOST_FILE
...
sed -i "s|$PROVIDER_ID_PATTERN|$PROVIDER_ID|g" $BASE_DIR/$DOCUMENTATION_FILE
