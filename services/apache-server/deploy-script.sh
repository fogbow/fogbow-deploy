...
VOLUME_SHIBBOLETH_PARAMENTERS_COMMAND=''
if [ "$AUTH_TYPE_CLASS" == "shibboleth" ]; then
  VOLUME_SHIBBOLETH_PARAMENTERS_COMMAND=" -v $DIR_PATH/$SERVICE_PROVIDER_CERTIFICATE_FILE_NAME:$CERTS_DIR/$SERVICE_PROVIDER_DOMAIN_NAME.crt " \
      " -v $DIR_PATH/$SERVICE_PROVIDER_CERTIFICATE_KEY_FILE_NAME:$SSL_DIR/$SERVICE_PROVIDER_DOMAIN_NAME.key " \
      " -v $DIR_PATH/$SHARED_FOLDER:$CONTAINER_BASE_DIR/$SHARED_FOLDER "
fi
...
if [ "$AUTH_TYPE_CLASS" == "shibboleth" ]; then
  ...
  sudo docker cp $AS_PUBLIC_KEY_NAME $CONTAINER_NAME:$SHIBBOLETH_AUTHENTICATION_APPLICATION_DIR
  sudo docker cp $SHIB_PRIVATE_KEY_NAME $CONTAINER_NAME:$SHIBBOLETH_AUTHENTICATION_APPLICATION_DIR
  sudo docker cp $SHIBBOLETH_AUTHENTICATION_APPLICATION_LOG4J_FILE $CONTAINER_NAME:$SHIBBOLETH_AUTHENTICATION_APPLICATION_DIR
  
  sudo docker exec $CONTAINER_NAME /bin/bash -c "bash $SHIBBOLETH_AUTHENTICATION_APPLICATION_DIR/bin/start-shib-app.sh &" &
fi
