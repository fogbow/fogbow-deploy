...
if [ "$AUTH_TYPE_CLASS" == "shibboleth" ]; then
  VOLUME_SHIBBOLETH_PARAMENTERS_COMMAND=$(cat <<EOF
      -v $DIR_PATH/$SERVICE_PROVIDER_CERTIFICATE_FILE_NAME:$SSL_DIR/$SERVICE_PROVIDER_DOMAIN_NAME".crt" \
      -v $DIR_PATH/$SERVICE_PROVIDER_CERTIFICATE_KEY_FILE_NAME:$SSL_DIR/$SERVICE_PROVIDER_DOMAIN_NAME".ket" \
      -v $DIR/"services"/$CONF_FILES_DIR_NAME/$SHARED_FOLDER:$CONTAINER_BASE_DIR/$SHARED_FOLDER
    EOF
  )
fi
