...
GENERAL_PUBLIC_KEY_PATH=$(grep $GENERAL_PUBLIC_KEY_PATTERN $CONF_FILES_DIR/$GENERAL_CONF_FILE_NAME | awk -F "=" '{print $2}')
GENERAL_PUBLIC_KEY_NAME=$(basename $GENERAL_PUBLIC_KEY_PATH)

echo "Copying $GENERAL_PUBLIC_KEY_NAME to $BASE_DIR/$GENERAL_PUBLIC_KEY_NAME"
yes | cp -f $GENERAL_PUBLIC_KEY_NAME $BASE_DIR/$GENERAL_PUBLIC_KEY_NAME
