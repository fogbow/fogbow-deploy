...
RAS_FILE_NAME=$(basename $RAS_CONF_FILE)

private_key_name=$(basename $GENERAL_PRIVATE_KEY_PATH)
public_key_name=$(basename $GENERAL_PUBLIC_KEY_PATH)

sed -i "s#.*$MANAGER_PRIVATE_KEY_PATTERN=.*#$MANAGER_PRIVATE_KEY_PATTERN=$CONTAINER_BASE_DIR/$CONTAINER_CONF_FILES_DIR/$private_key_name#" $BASE_DIR/$CONF_FILES_DIR_NAME/$RAS_FILE_NAME
sed -i "s#.*$MANAGER_PUBLIC_KEY_PATTERN=.*#$MANAGER_PUBLIC_KEY_PATTERN=$CONTAINER_BASE_DIR/$CONTAINER_CONF_FILES_DIR/$public_key_name#" $BASE_DIR/$CONF_FILES_DIR_NAME/$RAS_FILE_NAME
...
ENV_FEDNET_CONF_FILE=$BASE_DIR/$CONF_FILES_DIR_NAME/$FEDNET_FILE_NAME
...
DMZ_PRIVATE_KEY_PATH=$(grep $DMZ_PRIVATE_KEY_PATTERN $GENERAL_CONF_FILE_PATH | awk -F "=" '{print $2}')
DMZ_PRIVATE_KEY_NAME=$(basename $DMZ_PRIVATE_KEY_PATH)

echo "Moving dmz ssh private key $DMZ_PRIVATE_KEY_NAME to $BASE_CONF_FILES_DIR directory"

yes | cp -f $DMZ_PRIVATE_KEY_NAME $BASE_CONF_FILES_DIR

FEDNET_PERMISSION_FILE_PATH=$CONTAINER_BASE_DIR/$CONTAINER_CONF_FILES_DIR/$DMZ_PRIVATE_KEY_NAME
