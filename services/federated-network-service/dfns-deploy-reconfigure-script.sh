...
mv $AGENT_PRIVATE_KEY_FILE_PATH $CONF_FILE_DIR_PATH/$DFNS_AGENT_HOST_PRIVATE_KEY_FILE_NAME

DFNS_AGENT_HOST_PRIVATE_KEY_FILE_PATH_PATTERN="dfns_cluster_ssh_private_key_file"
DFNS_AGENT_HOST_PRIVATE_KEY_FILE_PATH=$(grep $DFNS_AGENT_HOST_PRIVATE_KEY_FILE_PATH_PATTERN $CONF_FILE_DIR_PATH/$SITE_CONF_FILE_NAME | awk -F "=" '{print $2}')
...
scp -i $DFNS_AGENT_HOST_PRIVATE_KEY_FILE_PATH $REMOTE_USER@$DFNS_AGENT_HOST:$AUTHORIZED_KEYS_FILE_PATH $AUTHORIZED_KEYS_FILE_NAME
...
if [ "$?" -ne "0" ]; then
    ...
    echo "Key already there: [$AGENT_HOST_PUBLIC_KEY]" >> $AUTHORIZED_KEYS_FILE_PATH
fi

#rm $AGENT_PUBLIC_KEY_FILE_PATH $AUTHORIZED_KEYS_FILE_PATH
