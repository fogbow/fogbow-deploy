...
SITE_CONF_FILE_NAME="./conf-files/multi-cloud/host.conf"
ANSIBLE_FILES_DIR_PATH="./ansible-playbook/multi-cloud"
...
SERVICE_HOST_IP=$(grep $SERVICE_HOST_IP_PATTERN $SITE_CONF_FILE_NAME | awk -F "=" '{print $2}')

SERVICE_HOST_PRIVATE_KEY_FILE_PATH_PATTERN="service_host_ssh_private_key_file"
SERVICE_HOST_PRIVATE_KEY_FILE_PATH=$(grep $SERVICE_HOST_PRIVATE_KEY_FILE_PATH_PATTERN $SITE_CONF_FILE_NAME | awk -F "=" '{print $2}')
